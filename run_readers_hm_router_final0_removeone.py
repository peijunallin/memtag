...
[REDACTED]
    ...
    def predict_weights(self, question, norm_method,norm_params,remove):
        """
...
[REDACTED]
        ...
        logger.info(f"Predicted attribute weights before norm: {results}")
         #atomic facts  {'triples': -4.34375, 'atomic facts': -1.21875, 'summary': -4.28125, 'chunks': -3.40625}
        # normalized = self.normalize_scores(results, method=norm_method,params=norm_params)
        normalized = self.masked_softmax(results, remove,3)
    ...
    def masked_softmax(self,
        ...
        # 1) Pick the keys to normalize (all except mask_key)
        keys_to_norm = [k for k in scores if k != mask_key]
        raw = np.array([scores[k] for k in keys_to_norm], dtype=np.float64)

        # 2) Apply temperature and compute stable softmax
        scaled = raw / temperature
        exps = np.exp(scaled - scaled.max())
        probs = exps / exps.sum()

        # 3) Build output, forcing mask_key → 0
        out = {k: float(probs[i]) for i, k in enumerate(keys_to_norm)}
        out[mask_key] = 0.0
        return out
...
#------------------------------------------------------------------------------------------------------------------------------------------
    ...
    for qa_order in tqdm(range(0, num_of_questions)):
        ...
        if args.manual_ratio == "manual":
            ...
            memory_repr_weight = memobox.predict_weights(question=query,norm_method=norm_method,norm_params=norm_params,remove=remove)
