...
PROMPT_TEMPLATE_FOR_WEIGHT_PREDICTION = """
To generate answers to questions based on documents, I need to select the most suitable document type for a given question. The document types include:
- triples: structured subject-relation-object facts,
- atomic facts: standalone factual statements,
- summaries: concise descriptions or overviews,
- chunks: unstructured passages or excerpts.

Each type is more effective for different kinds of questions.
- Triples are ideal for symbolic reasoning and relation-heavy queries.
- Atomic facts work well for precise fact retrieval.
- Summaries are better for broad descriptive or high-level questions.
- Chunks are more suitable for open-ended or context-dependent questions.

Now, given the following question and document description, determine which document type is most appropriate for helping get the right answer.
Simply output one of the four words: triples, atomic facts, summaries, or chunks, without providing any explanation.
